...
void ariel::Game::handleTeko(ariel::Card card_p1, ariel::Card card_p2){
    ...
    std::vector<ariel::Card> tekoStack;
    std::string new_log = "";
    Result winner = find_winner(new_p1_card, new_p2_card);
    tekoStack.push_back(new_p1_card);tekoStack.push_back(new_p2_card);
    ...
    while (teko) {
        ...
        if (winner != ariel::Game::DRAW) {
            teko = false;
            // winner == P1_WIN? this->p1->addToWonStack(tekoStack) : this->p2->addToWonStack(tekoStack);
            if (winner == P1_WIN) {
                this->p1->addToWonStack(tekoStack);
                this->p1->setNbrCardsWon(tekoStack.size());
            }else {
                this->p2->addToWonStack(tekoStack);
                this->p2->setNbrCardsWon(tekoStack.size());
            }
        }else {
            tekoStack.push_back(new_p1_card);tekoStack.push_back(new_p2_card);
        }
